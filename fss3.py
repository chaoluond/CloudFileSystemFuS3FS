...
import uuid
import signal
from FUS3FSPlugin import FUS3FSPlugin
import traceback
import time
from sns import SNS_HTTPServer, SNS_HTTPRequestHandler
from utilityFunc import *
import socket
from UTF8DecodingKey import *
from localcache import *
import copy
import datetime as dt
from TracebackLoggingThread import TracebackLoggingThread
import errno
import mimetypes
import gc
import json
...
class FUS3FS(LoggingMixIn, Operations):
    ...
    def __init__(self, options):
        ...
        logging.info("s3-retries: '%i'" % self.s3_retries)

        self.s3_retries_sleep = options.s3_retries_sleep # retry sleep in seconds
        logging.info("s3-retries-sleep: '%i' seconds" % self.s3_retries_sleep)
        ...
        logging.info("S3 bucket: '%s'" % self.s3_bucket_name)
        self.s3_prefix = s3url.path.strip('/')
        logging.info("S3 prefix (can be empty): '%s'" % self.s3_prefix)
        if self.s3_bucket_name == '':
            error_and_exit("The S3 bucket cannot be empty")
        self.sns_topic_arn = options.topic
        if self.sns_topic_arn:
            logging.info("AWS region for SNS and SQS: '" + self.aws_region + "'")
            logging.info("SNS topic ARN: '%s'" % self.sns_topic_arn)
        ...
        if self.sqs_queue_name:
            logging.info("SQS queue name: '%s'" % self.sqs_queue_name)
        if self.sqs_queue_name or self.new_queue:
            logging.info("SQS queue wait time (in seconds): '%i'" % self.queue_wait_time)
            logging.info("SQS queue polling interval (in seconds): '%i'" % self.queue_polling_interval)
        self.cache_entries = options.cache_entries
        logging.info("Cache entries: '%i'" % self.cache_entries)
        self.cache_mem_size = options.cache_mem_size * (1024 * 1024) # To convert MB to bytes
        logging.info("Cache memory size (in bytes): '%i'" % self.cache_mem_size)
        self.cache_disk_size = options.cache_disk_size * (1024 * 1024) # To convert MB to bytes
        logging.info("Cache disk size (in bytes): '%i'" % self.cache_disk_size)
        self.cache_on_disk = options.cache_on_disk # Bytes
        logging.info("Cache on disk if file size greater than (in bytes): '%i'" % self.cache_on_disk)
        self.cache_check_interval = options.cache_check # seconds
        logging.info("Cache check interval (in seconds): '%i'" % self.cache_check_interval)
        self.recheck_s3 = options.recheck_s3
        logging.info("Cache ENOENT rechecks S3: %s" % self.recheck_s3)
        self.aws_managed_encryption = options.aws_managed_encryption
        logging.info("AWS Managed Encryption enabled: %s" % self.aws_managed_encryption)
        ...
        if options.st_blksize:
            ...
            logging.info("getattr() st_blksize: '%i'" % self.st_blksize)
        ...
        if self.hostname:
            logging.info("Public hostname to listen to SNS HTTP notifications: '%s'" % self.hostname)
        self.sns_http_port = int(options.port or '0')
        if options.port:
            import M2Crypto # Required to check integrity of SNS HTTP notifications
            logging.info("TCP port to listen to SNS HTTP notifications: '%i'" % self.sns_http_port)
        self.s3_num = options.s3_num
        logging.info("Number of parallel S3 threads (0 to disable writeback): '%i'" % self.s3_num)
        self.download_num = options.download_num
        logging.info("Number of parallel downloading threads: '%i'" % self.download_num)


        self.download_retries_num = options.download_retries_num
        logging.info("Number download retry attempts: '%i'" % self.download_retries_num)
        self.download_retries_sleep = options.download_retries_sleep
        logging.info("Download retry sleep time seconds: '%i'" % self.download_retries_sleep)

        self.read_retries_num = options.read_retries_num
        logging.info("Number read retry attempts: '%i'" % self.read_retries_num)
        self.read_retries_sleep = options.read_retries_sleep
        logging.info("Read retry sleep time seconds: '%i'" % self.read_retries_sleep)


        self.prefetch_num = options.prefetch_num
        logging.info("Number of parallel prefetching threads: '%i'" % self.prefetch_num)
        self.buffer_size = options.buffer_size * 1024 # To convert KB to bytes
        logging.info("Download buffer size (in KB, 0 to disable buffering): '%i'" % self.buffer_size)
        self.buffer_prefetch = options.buffer_prefetch
        logging.info("Number of buffers to prefetch: '%i'" % self.buffer_prefetch)
        self.write_metadata = not options.no_metadata
        logging.info("Write metadata (file system attr/xattr) on S3: '%s'" % str(self.write_metadata))
        self.full_prefetch = options.prefetch
        logging.info("Download prefetch: '%s'" % str(self.full_prefetch))
        self.multipart_size = options.mp_size * (1024 * 1024) # To convert MB to bytes
        logging.info("Multipart size: '%s'" % str(self.multipart_size))
        self.multipart_num = options.mp_num
        logging.info("Multipart maximum number of parallel threads: '%s'" % str(self.multipart_num))
        self.multipart_retries = options.mp_retries
        logging.info("Multipart maximum number of retries per part: '%s'" % str(self.multipart_retries))
        self.default_expiration = options.expiration
        logging.info("Default expiration for signed URLs via xattrs: '%s'" % str(self.default_expiration))
        self.requester_pays = options.requester_pays
        logging.info("S3 Request Payer: '%s'" % str(self.requester_pays))
        ...
        logging.info("Cache path (on disk): '%s'" % cache_path)
        ...
        logging.info("Unique node ID: '%s'" % self.unique_id)
        ...
        if self.sqs_queue_name or self.new_queue:
            ...
            if self.new_queue:
                ...
                self.sqs_queue_name = '-'.join([ 'yas3fs',
                                               ...
                                               hostname,
                                               self.unique_id])
                self.sqs_queue_name = self.sqs_queue_name[:80]  
                self.sqs_queue_name = re.sub(r'-+', '-', self.sqs_queue_name)
                logging.info("Attempting to create SQS queue: " + self.sqs_queue_name)
            ...
            logging.info("SQS queue name (new): '%s'" % self.sqs_queue_name)
        ...
        if (options.with_plugin_file):
            self.plugin = FUS3FSPlugin.load_from_file(self, options.with_plugin_file, options.with_plugin_class)
        elif (options.with_plugin_class):
            self.plugin = FUS3FSPlugin.load_from_class(self, options.with_plugin_class)
    ...
    def check_threads(self, first=False):
        logging.debug("check_threads '%s'" % first)
        ...
        for i in range(self.s3_num):
            if thread_is_not_alive(self.s3_threads[i]):
                logging.debug("%s S3 thread #%i" % (display, i))
        ...
        for i in range(self.download_num):
            if thread_is_not_alive(self.download_threads[i]):
                logging.debug("%s download thread #%i" % (display, i))
        ...
        for i in range(self.prefetch_num):
            if thread_is_not_alive(self.prefetch_threads[i]):
                logging.debug("%s prefetch thread #%i" % (display, i))
        ...
        if self.sns_topic_arn:
            if thread_is_not_alive(self.publish_thread):
                logging.debug("%s publish thread" % display)
        ...
        if self.sqs_queue_name:
            if thread_is_not_alive(self.queue_listen_thread):
                logging.debug("%s queue listen thread" % display)
        ...
        if self.sns_http_port:
            if thread_is_not_alive(self.http_listen_thread):
                logging.debug("%s HTTP listen thread" % display)
        ...
        if thread_is_not_alive(self.check_cache_thread):
            logging.debug("%s check cache thread" % display)
    ...
    def init(self, path):
        ...
        if self.sqs_queue_name:
            logging.debug("Subscribing '%s' to '%s'" % (self.sqs_queue_name, self.sns_topic_arn))
            ...
            logging.debug('SNS SQS subscription = %s' % self.sqs_subscription)
    ...
    def signal_handler(self, signum, frame):
        logging.info("signal_handler RECEIVED %s", signum)
        if signum == signal.SIGINT:
            logging.info("interrupt RECEIVED SIGINT %s", signum)
            self.destroy('/')

        elif signum == signal.SIGHUP:
            logging.info("hangup RECEIVED SIGHUP %s", signum)

        logging.info("signal_handler DONE %s", signum)

    def flush_all_cache(self):
        logging.debug("flush_all_cache")
    ...
    def destroy(self, path):
        logging.debug("destroy '%s'" % (path))
        # Cleanup for unmount
        logging.info('File system unmount...')

        self.running = False

        if self.http_listen_thread:
            self.httpd.shutdown() # To stop HTTP listen thread
            logging.info("waiting for HTTP listen thread to shutdown...")
            self.http_listen_thread.join(5.0) # 5 seconds should be enough
            logging.info("HTTP listen thread ended")
            self.sns.unsubscribe(self.http_subscription)
            logging.info("Unsubscribed SNS HTTP endpoint")
        if self.queue_listen_thread:
            self.sqs_queue_name = None # To stop queue listen thread
            logging.info("waiting for SQS listen thread to shutdown...")
            self.queue_listen_thread.join(self.queue_wait_time + 1.0)
            logging.info("SQS listen thread ended")
            self.sns.unsubscribe(self.sqs_subscription)
            logging.info("Unsubscribed SNS SQS endpoint")
            if self.new_queue:
                if self.sqs.delete_queue(self.queue):
                    logging.info("New queue deleted")
                else:
                    logging.error("New queue was not deleted")
        ...
        if self.sns_topic_arn:
            ...
            logging.info("waiting for SNS publish thread to shutdown...")
            self.publish_thread.join(2.0) # 2 seconds should be enough
        if  self.cache_entries:
            self.cache_entries = 0 # To stop memory thread
            logging.info("waiting for check cache thread to shutdown...")
    ...
    def listen_for_messages_over_http(self):
        logging.info("Listening on: '%s'" % self.http_listen_url)
    ...
    def listen_for_messages_over_sqs(self):
        logging.info("Listening on queue: '%s'" % self.queue.name)
        while self.sqs_queue_name:
            ...
            logging.debug("Got %i messages from SQS" % len(messages))
            if messages:
                for m in messages:
    ...
    def invalidate_cache(self, path, etag=None):
        logging.debug("invalidate_cache '%s' '%s'" % (path, etag))
    ...
    def delete_cache(self, path):
        logging.debug("delete_cache '%s'" % (path))
        with self.cache.get_lock(path):
            self.cache.delete(path)
            self.reset_parent_readdir(path)

    def process_message(self, messages):
        logging.debug("process_message '%s'" % (messages))
        c = json.loads(messages)
        if c[0] == self.unique_id:
            # discard message coming from itself
            logging.debug("process message from self discarded '%s'"%(c))
            return
        ...
        if c[1] in ( 'mkdir', 'mknod', 'symlink' ) and c[2] != None:
            ...
            with self.cache.lock:
                ...
                logging.info("S3 bucket: '%s'" % self.s3_bucket_name)
                self.s3_prefix = s3url.path.strip('/')
                logging.info("S3 prefix: '%s'" % self.s3_prefix)
                try:
                    self.s3_bucket = self.s3.get_bucket(self.s3_bucket_name, headers=self.default_headers, validate=False)
                    self.s3_bucket.key_class = UTF8DecodingKey
                except boto.exception.S3ResponseError, e:
    ...
    def publish_messages(self):
        while self.sns_topic_arn:
            try:
                ...
                logging.exception(e)
                logging.error("publish exception: " + full_message.encode('ascii'))
                raise e


    def publish(self, message):
        if self.sns_topic_arn:
            logging.debug("publish '%s'" % (message))
    ...
    def check_status(self):
        ...
        while self.cache_entries:
            ...
            for i in range(0, self.s3_num):
                s3q += self.s3_queue[i].qsize()
            logging.info("entries, mem_size, disk_size, download_queue, prefetch_queue, s3_queue: %i, %i, %i, %i, %i, %i"
                        % (num_entries, mem_size, disk_size,
                           self.download_queue.qsize(), self.prefetch_queue.qsize(), s3q))

            logging.info("multipart_uploads_in_progress = " + str(self.multipart_uploads_in_progress))

            if True:
                logging.debug("new_locks, unused_locks: %i, %i"
                             % (len(self.cache.new_locks), len(self.cache.unused_locks)))
                (threshold0, threshold1, threshold2) = gc.get_threshold()
                (count0, count1, count2) = gc.get_count()
                logging.debug("gc count0/threshold0, count1/threshold1, count2/threshold2: %i/%i, %i/%i, %i/%i"
    ...
    def check_cache_size(self):
        ...
        logging.debug("check_cache_size")

        while self.cache_entries:

            logging.debug("check_cache_size get_memory_usage")
            num_entries, mem_size, disk_size = self.cache.get_memory_usage()
            ...
            if purge:
                ...
                with self.cache.get_lock(path):
                    if self.cache.has(path): # Path may be deleted before I acquire the lock
                        logging.debug("check_cache_size purge: '%s' '%s' ?" % (store, path))
                        data = self.cache.get(path, 'data')
                        full_delete = False
                        if (not data) or (data and (store == '' or data.store == store) and (not data.has('open')) and (not data.has('change'))):
                            if store == '':
                                logging.debug("check_cache_size purge: '%s' '%s' OK full" % (store, path))
                                self.cache.delete(path) # Remove completely from cache
                                full_delete = True
                            elif data:
                                logging.debug("check_cache_size purge: '%s' '%s' OK data" % (store, path))
                                self.cache.delete(path, 'data') # Just remove data
                            else:
                                logging.debug("check_cache_size purge: '%s' '%s' KO no data" % (store, path))
                        else:
                            logging.debug("check_cache_size purge: '%s' '%s' KO data? %s open? %s change? %s"
                ...
                for path in self.cache.unused_locks.keys():
                    logging.debug("check_cache_size purge unused lock: '%s'" % (path))
                    try:
                        with self.cache.lock and self.cache.new_locks[path]:
                            del self.cache.new_locks[path]
                            logging.debug("check_cache_size purge unused lock: '%s' deleted" % (path))
                    except KeyError:
                        pass
                    try:
                        del self.cache.unused_locks[path]
                        logging.debug("check_cache_size purge unused lock: '%s' removed from list" % (path))
                    except KeyError:
                        pass
                # Look for unused locks to be removed at next iteration (if still "new")
                for path in self.cache.new_locks.keys():
                    logging.debug("check_cache_size purge unused lock: '%s' added to list" % (path))
    ...
    def add_to_parent_readdir(self, path):
        logging.debug("add_to_parent_readdir '%s'" % (path))
        (parent_path, dir) = os.path.split(path)
        logging.debug("add_to_parent_readdir '%s' parent_path '%s'" % (path, parent_path))
    ...
    def remove_from_parent_readdir(self, path):
        logging.debug("remove_from_parent_readdir '%s'" % (path))
        (parent_path, dir) = os.path.split(path)
        logging.debug("remove_from_parent_readdir '%s' parent_path '%s'" % (path, parent_path))
        with self.cache.get_lock(parent_path):
            dirs = self.cache.get(parent_path, 'readdir')
            if dirs != None:
                try:
    ...
    def reset_parent_readdir(self, path):
        logging.debug("reset_parent_readdir '%s'" % (path))
        (parent_path, dir) = os.path.split(path)
        logging.debug("reset_parent_readdir '%s' parent_path '%s'" % (path, parent_path))
    ...
    def has_elements(self, iter, num=1):
        logging.debug("has_element '%s' %i" % (iter, num))
        c = 0
        for k in iter:
            logging.debug("has_element '%s' -> '%s'" % (iter, k))
            path = k.name[len(self.s3_prefix):]
            if not self.cache.is_deleting(path):
                c += 1
            if c >= num:
                logging.debug("has_element '%s' OK" % (iter))
                return True
        logging.debug("has_element '%s' KO" % (iter))
        return False

    def folder_has_contents(self, path, num=1):
        logging.debug("folder_has_contents '%s' %i" % (path, num))
    ...
    def get_key(self, path, cache=True):
        if self.cache.is_deleting(path):
            logging.debug("get_key path '%s' is deleting -- returning None" % (path))
            return None

        if cache and self.cache.is_ready(path):
            key = self.cache.get(path, 'key')
            if key:
                logging.debug("get_key from cache '%s'" % (path))
                return key
        logging.debug("get_key %s", path)
        look_on_S3 = True
        ...
        if look_on_S3:
            ...
            if key:
                ...
                logging.debug("get_key to cache '%s'" % (path))
                ###self.cache.delete(path) ### ???
                ###self.cache.add(path)
            ...
            logging.debug("get_key not on S3 '%s'" % (path))
        if not key:
            logging.debug("get_key no '%s'" % (path))
        return key

    def get_metadata(self, path, metadata_name, key=None):
        logging.debug("get_metadata -> '%s' '%s' '%s'" % (path, metadata_name, key))
        with self.cache.get_lock(path): # To avoid consistency issues, e.g. with a concurrent purge
            ...
            if metadata_values == None:
                ...
                if not key:
                    if path == '/': # First time mount of a new file system
                        ...
                        logging.debug("get_metadata -> '%s' '%s' First time mount"
                                     % (path, metadata_name))
                        ...
                        if not self.folder_has_contents(path):
                            ...
                            logging.debug("get_metadata '%s' '%s' no S3 return None"
                                         % (path, metadata_name))
                            return None
                else:
                    logging.debug("get_metadata '%s' '%s' '%s' S3 found"
            ...
            logging.debug("get_metadata <- '%s' '%s' '%s' '%s'" % (path, metadata_name, key, metadata_values))
            return metadata_values

    def set_metadata(self, path, metadata_name=None, metadata_values=None, key=None):
        logging.debug("set_metadata '%s' '%s' '%s'" % (path, metadata_name, key))
        with self.cache.get_lock(path):
            ...
            if self.write_metadata and (key or (not data) or (data and not data.has('change'))):
                ...
                if not key:
                    key = self.get_key(path)
                    logging.debug("set_metadata '%s' '%s' '%s' Key" % (path, metadata_name, key))
                new_key = False
                    ...
                    if metadata_name:
                        ...
                        if values == None or not any(values):
                            ...
                            try:
                                ...
                                logging.info("set_metadata '%s' '%s' '%s' cannot decode unicode, not written on S3"
                                            % (path, metadata_name, key))
                                pass # Ignore the binary values - something better TODO ???
                    if (not data) or (data and (not data.has('change'))):
                        logging.debug("set_metadata '%s' '%s' S3" % (path, key))
                        pub = [ 'md', metadata_name, path ]
                        if new_key:
                            logging.debug("set_metadata '%s' '%s' S3 new key" % (path, key))
                ...
                if metadata_name == 'attr' and metadata_values == None:
                    logging.debug("set_metadata: 'change' already in progress, setting FSData.props[invoke_after_change] lambda for self.set_metadata("+path+",attr)")
                    data.set('invoke_after_change',(lambda path: self.set_metadata(path,'attr')))

    def getattr(self, path, fh=None):
        logging.debug("getattr -> '%s' '%s'" % (path, fh))
        if self.cache.is_deleting(path):
            logging.debug("getattr path '%s' is deleting -- throwing ENOENT" % (path))
        ...
        with self.cache.get_lock(path): # To avoid consistency issues, e.g. with a concurrent purge
            ...
            recheck_s3 = False
            if self.cache.is_empty(path):
                logging.debug("getattr <- '%s' '%s' cache ENOENT" % (path, fh))
                if self.recheck_s3:
                    cache = False
                    recheck_s3 = True
                    logging.debug("getattr rechecking on s3 <- '%s' '%s' cache ENOENT" % (path, fh))
                else:
                    raise FuseOSError(errno.ENOENT)

            attr = self.get_metadata(path, 'attr')
            if attr == None:
                logging.debug("getattr <- '%s' '%s' ENOENT" % (path, fh))
                raise FuseOSError(errno.ENOENT)
            ...
            logging.debug("getattr <- '%s' '%s' '%s'" % (path, fh, attr))
            return attr
    ...
    def readdir(self, path, fh=None):
        ...
        if self.cache.is_deleting(path):
            logging.debug("readdir path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("readdir '%s' '%s' ENOENT" % (path, fh))
                raise FuseOSError(errno.ENOENT)
            self.cache.add(path)
            dirs = self.cache.get(path, 'readdir')

            if not dirs:
                logging.debug("readdir '%s' '%s' no cache" % (path, fh))
                full_path = self.join_prefix(path)
                if full_path == '.':
                    full_path = ''
                elif full_path != '' and full_path[-1] != '/':
                    full_path += '/'
                logging.debug("readdir '%s' '%s' S3 list '%s'" % (path, fh, full_path))
                ...
                for k in key_list:
                    ...
                    logging.debug("readdir '%s' '%s' S3 list key '%s'" % (path, fh, k))
            ...
            logging.debug("readdir '%s' '%s' '%s'" % (path, fh, dirs))
            return dirs
    ...
    def mkdir(self, path, mode):
        ...
        with self.cache.get_lock(path):
            if self.cache.is_not_empty(path):
                logging.debug("mkdir cache '%s' EEXIST" % self.cache.get(path))
                raise FuseOSError(errno.EEXIST)
            k = self.get_key(path)
            if k and path != '/':
                logging.debug("mkdir key '%s' EEXIST" % self.cache.get(path))
                raise FuseOSError(errno.EEXIST)
            now = get_current_time()
            uid, gid = get_uid_gid()
            attr = { 'st_uid': uid,
            ...
            if path != '/' or self.write_metadata:
                ...
                logging.debug("mkdir '%s' '%s' '%s' S3" % (path, mode, k))
    ...
    def symlink(self, path, link):
        logging.debug("symlink '%s' '%s'" % (path, link))
        with self.cache.get_lock(path):
            if self.cache.is_not_empty(path):
                logging.debug("symlink cache '%s' '%s' EEXIST" % (path, link))
                raise FuseOSError(errno.EEXIST)
            k = self.get_key(path)
            if k:
                logging.debug("symlink key '%s' '%s' EEXIST" % (path, link))
                raise FuseOSError(errno.EEXIST)
            now = get_current_time()
            uid, gid = get_uid_gid()
            attr = {}
            ...
            self.cache.set(path, 'key', k)
            self.add_to_parent_readdir(path)
            logging.debug("symlink '%s' '%s' '%s' S3" % (path, link, k))
    ...
    def check_data(self, path):
        logging.debug("check_data '%s'" % (path))
        with self.cache.get_lock(path):
            data = self.cache.get(path, 'data')
            if not data or data.has('new'):
                k = self.get_key(path)
                if not k:
                    logging.debug("check_data '%s' no key" % (path))
                    return False
                if not data:
    ...
    def download_data(self, path, start, end):
        ...
        logging.debug("download_data '%s' %i-%i [thread '%s']" % (path, start, end, thread_name))

        original_key = self.get_key(path)
        if original_key == None:
            logging.debug("download_data no key (before) '%s' [thread '%s']"
                             % (path, thread_name))
            return
        logging.debug("type k = '%s'" % type(original_key))
        logging.debug(" dir k = '%s'" % dir(original_key))
        logging.debug("     k = '%s'" % original_key)
        key = copy.copy(original_key)

        if start > (key.size - 1):
            logging.debug("download_data EOF '%s' %i-%i [thread '%s']" % (path, start, end, thread_name))
            return

        with self.cache.get_lock(path):
            data = self.cache.get(path, 'data')
            if not data:
                logging.debug("download_data no data (before) '%s' [thread '%s']" % (path, thread_name))
                return
            data_range = data.get('range')
            if not data_range:
                logging.debug("download_data no range (before) '%s' [thread '%s']"
                             % (path, thread_name))
                return
            new_interval = [start, min(end, key.size - 1)]
            if data_range.interval.contains(new_interval): ### Can be removed ???
                logging.debug("download_data '%s' %i-%i [thread '%s'] already downloaded"
                             % (path, start, end, thread_name))
                return
            else:
                for i in data_range.ongoing_intervals.itervalues():
                    if i[0] <= new_interval[0] and i[1] >= new_interval[1]:
                        logging.debug("download_data '%s' %i-%i [thread '%s'] already downloading"
                                     % (path, start, end, thread_name))
                        return
        ...
        while retry:
            ...
            if key is None:
                logging.warn("download_data 'key' is None!.. exiting retry loop")
                break
            ...
            logging.debug("download_data range '%s' '%s' [thread '%s'] max: %i sleep: %i retries: %i" % (path, range_headers, thread_name, self.download_retries_num, self.download_retries_sleep, retriesAttempted))
            try:
                if debug:
                    n1=dt.datetime.now()
                ...
                logging.exception(e)
                logging.info("download_data error '%s' %i-%i [thread '%s'] -> retrying max: %i sleep: %i retries: %i" % (path, start, end, thread_name, self.download_retries_num, self.download_retries_sleep, retriesAttempted))
        ...
        if debug:
            ...
            logging.debug("download_data done '%s' %i-%i [thread '%s'] elapsed %.6f" % (path, start, end, thread_name, elapsed))

        with self.cache.get_lock(path):
                data = self.cache.get(path, 'data')
                if not data:
                    logging.debug("download_data no data (after) '%s' [thread '%s']" % (path, thread_name))
                    return
                data_range = data.get('range')
                if not data_range:
                    logging.debug("download_data no range (after) '%s' [thread '%s']" % (path, thread_name))
                    return
                del data_range.ongoing_intervals[thread_name]
                if not bytes:
                    length = 0
                    logging.debug("download_data no bytes '%s' [thread '%s']" % (path, thread_name))
                else:
                    length = len(bytes)
                    logging.debug("download_data %i bytes '%s' [thread '%s']" % (length, path, thread_name))
                if length > 0:
                    with data.get_lock():
                        no_content = False
        ...
        logging.debug("download_data end '%s' %i-%i [thread '%s']" % (path, start, end, thread_name))
        ...
        with self.cache.get_lock(path):
            ...
            if data_range:
                if data_range.interval.contains([0, key.size - 1]): # -1 ???
                    ...
                    logging.debug("download_data all ended '%s' [thread '%s']" % (path, thread_name))
    ...
    def do_cmd_on_s3_now(self, key, pub, action, args, kargs):
        logging.debug("do_cmd_on_s3_now action '%s' key '%s' args '%s' kargs '%s'" % (action, key, args, kargs))
        ...
        try:
            if action == 'delete':
                ...
                try:
                    ...
                    logging.error("set_contents_from_file IOError on " + str(data))
                    raise e
                ...
                logging.debug("Multipart-upload Key Sizes '%s' local: %i remote: %i" %(key, full_size, uploaded_key.size))
                if full_size != uploaded_key.size:
                     logging.error("Multipart-upload Key Sizes do not match for '%s' local: %i remote: %i" %(key, full_size, uploaded_key.size))
                ...
                logging.error("do_cmd_on_s3_now Unknown action '%s'" % action)
        ...
        logging.debug("do_cmd_on_s3_now action '%s' key '%s' args '%s' kargs '%s' done" % (action, key, args, kargs))
        return pub
    ...
    def do_cmd_on_s3_now_w_retries(self, key, pub, action, args, kargs, retries = 1):
        ...
        for tries in range(1, retries +1):
            ...
            try:
                logging.debug("do_cmd_on_s3_now_w_retries try %s action '%s' key '%s' args '%s' kargs '%s'" % (tries, action, key, args, kargs))
                return self.do_cmd_on_s3_now(key, pub, action, args, kargs)
            except Exception, e:
                last_exception = e

        logging.error("do_cmd_on_s3_now_w_retries FAILED '%s' key '%s' args '%s' kargs '%s'" % (action, key, args, kargs))
    ...
    def readlink(self, path):
        ...
        if self.cache.is_deleting(path):
            logging.debug("readlink path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("readlink '%s' ENOENT" % (path))
                raise FuseOSError(errno.ENOENT)
            self.cache.add(path)
            ...
            data = self.cache.get(path, 'data')
            if data == None:
                logging.error("readlink '%s' no data ENOENT" % (path))
        ...
        if data_range:
            ...
            while True:
                logging.debug("readlink wait '%s'" % (path))
                data_range.wait()
                logging.debug("readlink awake '%s'" % (path))
                data_range = data.get('range')
                if not data_range:
                    break
        data.open()
    ...
    def rmdir(self, path):
        ...
        if self.cache.is_deleting(path):
            logging.debug("rmdir path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("rmdir '%s' cache ENOENT" % (path))
                raise FuseOSError(errno.ENOENT)
            k = self.get_key(path)
            if not k and not self.cache.has(path) and not self.folder_has_contents(path):
                logging.debug("rmdir '%s' S3 ENOENT" % (path))
                raise FuseOSError(errno.ENOENT)
            dirs = self.cache.get(path, 'readdir')
            if dirs == None:
                if self.folder_has_contents(path, 2): # There is something inside the folder
                    logging.debug("rmdir '%s' S3 ENOTEMPTY" % (path))
                    raise FuseOSError(errno.ENOTEMPTY)
            else:
                if len(dirs) > 2:
                    logging.debug("rmdir '%s' cache ENOTEMPTY" % (path))
                    raise FuseOSError(errno.ENOTEMPTY)
            ###k.delete()
            ...
            if k:
                logging.debug("rmdir '%s' '%s' S3" % (path, k))
                pub = [ 'rmdir', path ]
    ...
    def truncate(self, path, size):
        ...
        if self.cache.is_deleting(path):
            logging.debug("truncate path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("truncate '%s' '%i' ENOENT" % (path, size))
                raise FuseOSError(errno.ENOENT)
            self.cache.add(path)
            if not self.check_data(path):
                logging.debug("truncate '%s' '%i' ENOENT" % (path, size))
            ...
            while True:
                data = self.cache.get(path, 'data')
                if not data:
                    logging.error("truncate '%s' '%i' no data ENOENT" % (path, size))
                    raise FuseOSError(errno.ENOENT) # ??? That should not happen
                data_range = data.get('range')
                if not data_range:
                    break
                ...
                logging.debug("truncate wait '%s' '%i'" % (path, size))
                data_range.wait()
                logging.debug("truncate awake '%s' '%i'" % (path, size))
    ...
    def rename(self, path, new_path):
        logging.debug("rename '%s' '%s'" % (path, new_path))

        if self.cache.is_deleting(path):
            logging.debug("rename path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("rename '%s' '%s' ENOENT no '%s' from cache" % (path, new_path, path))
                raise FuseOSError(errno.ENOENT)
            key = self.get_key(path)
            if not key and not self.cache.has(path):
                logging.debug("rename '%s' '%s' ENOENT no '%s'" % (path, new_path, path))
                raise FuseOSError(errno.ENOENT)
            ...
            if not new_parent_key and not self.folder_has_contents(new_parent_path):
                logging.debug("rename '%s' '%s' ENOENT no parent path '%s'" % (path, new_path, new_parent_path))
                raise FuseOSError(errno.ENOENT)
    ...
    def rename_path(self, path, new_path):
        logging.debug("rename_path '%s' -> '%s'" % (path, new_path))
    ...
    def rename_item(self, path, new_path, dir=False):
        logging.debug("rename_item '%s' -> '%s' dir?%s" % (path, new_path, dir))
        source_path = path
    ...
    def rename_on_s3(self, key, source_path, target_path, dir):
        logging.debug("rename_on_s3 '%s' '%s' -> '%s' dir?%s" % (key, source_path, target_path, dir))
        # Otherwise we loose the Content-Type with S3 Copy
        key.metadata['Content-Type'] = key.content_type
    ...
    def mknod(self, path, mode, dev=None):
        logging.debug("mknod '%s' '%i' '%s'" % (path, mode, dev))
        with self.cache.get_lock(path):
            if self.cache.is_not_empty(file):
                logging.debug("mknod '%s' '%i' '%s' cache EEXIST" % (path, mode, dev))
                raise FuseOSError(errno.EEXIST)
            k = self.get_key(path)
            if k:
                logging.debug("mknod '%s' '%i' '%s' key EEXIST" % (path, mode, dev))
                raise FuseOSError(errno.EEXIST)
            self.cache.add(path)
            now = get_current_time()
            uid, gid = get_uid_gid()
    ...
    def unlink(self, path):
        ...
        if self.cache.is_deleting(path):
            logging.debug("unlink path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("unlink '%s' ENOENT" % (path))
                raise FuseOSError(errno.ENOENT)
            k = self.get_key(path)
            ...
            self.remove_from_parent_readdir(path)
            if k:
                logging.debug("unlink '%s' '%s' S3" % (path, k))
                ###k.delete()
    ...
    def create(self, path, mode, fi=None):
        logging.debug("create '%s' '%i' '%s'" % (path, mode, fi))
    ...
    def open(self, path, flags):
        ...
        with self.cache.get_lock(path):
            ...
            logging.debug("open '%s' '%i' '%s'" % (path, flags, self.cache.get(path, 'data').get('open')))
        return 0

    def release(self, path, flags):
        logging.debug("release '%s' '%i'" % (path, flags))

        if self.cache.is_deleting(path):
            logging.debug("release path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("release '%s' '%i' ENOENT" % (path, flags))
                raise FuseOSError(errno.ENOENT)
            data = self.cache.get(path, 'data')
            if data:
                ...
                logging.debug("release '%s' '%i' '%s'" % (path, flags, data.get('open')))
            else:
                logging.debug("release '%s' '%i'" % (path, flags))
        return 0

    def read(self, path, length, offset, fh=None):
        logging.debug("read '%s' '%i' '%i' '%s'" % (path, length, offset, fh))

        if self.cache.is_deleting(path):
            logging.debug("read path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        if not self.cache.has(path) or self.cache.is_empty(path):
            logging.debug("read '%s' '%i' '%i' '%s' ENOENT" % (path, length, offset, fh))
            raise FuseOSError(errno.ENOENT)
        ...
        while retry:
            ...
            if retriesAttempted > self.read_retries_num:
                logging.error("read '%s' '%i' '%i' '%s' max read retries exceeded max: %i sleep: %i retries: %i, raising FuseOSError(errno.EIO) ''" % (path, length, offset, fh, self.read_retries_num, self.read_retries_sleep, retriesAttempted))
                retry = False
                self.invalidate_cache(path)
            ...
            data = self.cache.get(path, 'data')
            if not data:
                logging.debug("read '%s' '%i' '%i' '%s' no data" % (path, length, offset, fh))
                return '' # Something better ???
            data_range = data.get('range')
            if data_range == None:
                logging.debug("read '%s' '%i' '%i' '%s' no range" % (path, length, offset, fh))
                break
            ...
            if offset > end_interval:
                logging.debug("read '%s' '%i' '%i' '%s' offset=%i > end_interval=%i" %((path, length, offset, fh, offset, end_interval)))
                return '' # Is this ok ???
            ...
            if data_range.interval.contains(read_interval):
                if self.buffer_prefetch:
                    ...
                    logging.debug("download prefetch '%s' '%i' '%i'" % (path, prefetch_start, prefetch_length))
                ...
                logging.debug("read '%s' '%i' '%i' '%s' in range" % (path, length, offset, fh))
                break
            else:
                if retriesAttempted > 1:
                    logging.debug('%d retries' % (retriesAttempted))
                    time.sleep(self.read_retries_sleep)
                
                logging.debug("read '%s' '%i' '%i' '%s' out of range" % (path, length, offset, fh))
                self.enqueue_download_data(path, offset, length)


            logging.debug("read wait '%s' '%i' '%i' '%s'" % (path, length, offset, fh))
            data_range.wait()
            logging.debug("read awake '%s' '%i' '%i' '%s'" % (path, length, offset, fh))
            # update atime just in the cache ???
        with data.get_lock():
            if not data.content:
                logging.debug("read '%s' '%i' '%i' '%s' no content" % (path, length, offset, fh))
                return '' # Something better ???
            data.content.seek(offset)
            return data.content.read(length)

    def write(self, path, new_data, offset, fh=None):
        logging.debug("write '%s' '%i' '%i' '%s'" % (path, len(new_data), offset, fh))

        if self.cache.is_deleting(path):
            logging.debug("write path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        if not self.cache.has(path) or self.cache.is_empty(path):
            logging.debug("write '%s' '%i' '%i' '%s' ENOENT" % (path, len(new_data), offset, fh))
            raise FuseOSError(errno.ENOENT)
        if isinstance(new_data, unicode): # Fix for unicode
            logging.debug("write '%s' '%i' '%i' '%s' unicode fix" % (path, len(new_data), offset, fh))
        ...
        if data_range:
            self.enqueue_download_data(path)
            while data_range:
                logging.debug("write wait '%s' '%i' '%i' '%s'" % (path, len(new_data), offset, fh))
                data_range.wait()
                logging.debug("write awake '%s' '%i' '%i' '%s'" % (path, len(new_data), offset, fh))
                data_range = data.get('range')

        with data.get_lock():
            if not data.content:
                logging.info("write awake '%s' '%i' '%i' '%s' no content" % (path, len(new_data), offset, fh))
                return 0
            logging.debug("write '%s' '%i' '%i' '%s' '%s' content" % (path, len(new_data), offset, fh, data.content.name.decode('utf-8')))
            data.content.seek(offset)
    ...
    def upload_to_s3(self, path, data):
        logging.debug("upload_to_s3 '%s'" % path)
        k = self.get_key(path)
        if not k: # New key
            k = UTF8DecodingKey(self.s3_bucket)
            k.key = self.join_prefix(path)
        ...
        logging.debug("multipart test: key '%s' mp-num '%s' st_size '%s' mp-size '%s'" %(path, self.multipart_num, attr['st_size'], self.multipart_size))
        if self.multipart_num > 0:
            full_size = attr['st_size']
            if full_size > self.multipart_size:
                logging.debug("upload_to_s3 '%s' '%s' '%s' S3 multipart" % (path, k, mimetype))
                cmds = [ [ 'multipart_upload', [ k.name, data, full_size, headers, k.metadata ] ] ]
                written = True
        if not written:
            logging.debug("upload_to_s3 '%s' '%s' '%s' S3" % (path, k, mimetype))
        ...
        logging.debug("upload_to_s3 '%s' done" % path)

    def multipart_upload(self, key_path, data, full_size, headers, metadata):

        logging.debug("multipart_upload '%s' '%s' '%s' '%s'" % (key_path, data, full_size, headers))
        part_num = 0
        part_pos = 0
        part_queue = Queue.Queue()
        multipart_size = max(self.multipart_size, full_size / 100) # No more than 100 parts...
        logging.debug("multipart_upload '%s' multipart_size '%s'" % (key_path, multipart_size))
        while part_pos < full_size:
            ...
            part_pos += part_size
            logging.debug("part from %i for %i" % (part_pos, part_size))
        logging.debug("initiate_multipart_upload '%s' '%s'" % (key_path, headers))
        num_threads = min(part_num, self.multipart_num)
        logging.debug("multipart_upload '%s' num_threads '%s'" % (key_path, num_threads))
        ...
        for i in range(num_threads):
            ...
            t.demon = True
            t.start()
            logging.debug("multipart_upload thread '%i' started" % i)
        logging.debug("multipart_upload all threads started '%s' '%s' '%s'" % (key_path, data, headers))
        part_queue.join()
        logging.debug("multipart_upload all threads joined '%s' '%s' '%s'" % (key_path, data, headers))
        if len(mpu.get_all_parts()) == part_num:
            logging.debug("multipart_upload ok '%s' '%s' '%s'" % (key_path, data, headers))
            new_key = mpu.complete_upload()
            self.multipart_uploads_in_progress -= 1
        else:
            logging.debug("multipart_upload cancel '%s' '%s' '%s' '%i' != '%i'" % (key_path, data, headers, len(mpu.get_all_parts()), part_num))
            mpu.cancel_upload()
            new_key = None
            self.multipart_uploads_in_progress -= 1
        return new_key
    ...
    def part_upload(self, mpu, part_queue):
        ...
        try:
            while (True):
                logging.debug("trying to get a part from the queue")
                [ num, part ] = part_queue.get(False)
                for retry in range(self.multipart_retries):
                    logging.debug("begin upload of part %i retry %i part__ %s" % (num, retry, str(part.__dict__)))
                    try:
                        ...
                        logging.exception(e)
                        logging.info("error during multipart upload part %i retry %i part__ %s : %s"
                                    % (num, retry, str(part.__dict__), sys.exc_info()[0]))
                        time.sleep(self.s3_retries_sleep) # Better wait N seconds before retrying
                logging.debug("end upload of part %i retry %i part__ %s" % (num, retry, str(part.__dict__)))
                part_queue.task_done()
        except Queue.Empty:
            logging.debug("the queue is empty")

    def chmod(self, path, mode):
        logging.debug("chmod '%s' '%i'" % (path, mode))

        if self.cache.is_deleting(path):
            logging.debug("chmod path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("chmod '%s' '%i' ENOENT" % (path, mode))
                raise FuseOSError(errno.ENOENT)
            attr = self.get_metadata(path, 'attr')
            if attr < 0:
                return attr
    ...
    def chown(self, path, uid, gid):
        logging.debug("chown '%s' '%i' '%i'" % (path, uid, gid))

        if self.cache.is_deleting(path):
            logging.debug("chown path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("chown '%s' '%i' '%i' ENOENT" % (path, uid, gid))
                raise FuseOSError(errno.ENOENT)
            attr = self.get_metadata(path, 'attr')
            if attr < 0:
                return attr
    ...
    def utimens(self, path, times=None):
        ...
        if self.cache.is_deleting(path):
            logging.debug("utimens path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("utimens '%s' '%s' ENOENT" % (path, times))
                raise FuseOSError(errno.ENOENT)
            now = get_current_time()
    ...
    def getxattr(self, path, name, position=0):
        logging.debug("getxattr '%s' '%s' '%i'" % (path, name, position))

        if name in ['yas3fs.bucket', 'user.yas3fs.bucket']:
            return self.s3_bucket_name

        if self.cache.is_deleting(path):
            logging.debug("getxattr path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        if self.cache.is_empty(path):
            logging.debug("getxattr '%s' '%s' '%i' ENOENT" % (path, name, position))
            raise FuseOSError(errno.ENOENT)
    ...
    def listxattr(self, path):
        logging.debug("listxattr '%s'" % (path))

        if self.cache.is_deleting(path):
            logging.debug("listxattr path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        if self.cache.is_empty(path):
            logging.debug("listxattr '%s' ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)
        xattr = self.get_metadata(path, 'xattr')
        if xattr == None:
            logging.debug("listxattr <- '%s' '%s' '%i' ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)
        return set(self.yas3fs_xattrs + xattr.keys())

    def removexattr(self, path, name):
        logging.debug("removexattr '%s''%s'" % (path, name))

        if self.cache.is_deleting(path):
            logging.debug("removexattr path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)


        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("removexattr '%s' '%s' ENOENT" % (path, name))
                raise FuseOSError(errno.ENOENT)
            ...
            try:
                ...
                if name not in self.yas3fs_xattrs:
                    logging.debug("removexattr '%s' '%s' should ENOATTR" % (path, name))
                    if self.darwin:
                        raise FuseOSError(errno.ENOENT) # Should return ENOATTR
                    else:
                        return '' # Should return ENOATTR
    ...
    def setxattr(self, path, name, value, options, position=0):
        logging.debug("setxattr '%s' '%s'" % (path, name))

        if self.cache.is_deleting(path):
            logging.debug("setxattr path '%s' is deleting -- throwing ENOENT" % (path))
            raise FuseOSError(errno.ENOENT)

        with self.cache.get_lock(path):
            if self.cache.is_empty(path):
                logging.debug("setxattr '%s' '%s' ENOENT" % (path, name))
                raise FuseOSError(errno.ENOENT)
            if name in self.yas3fs_xattrs and name not in ['user.yas3fs.expiration']:
                return 0 # Do nothing
            xattr = self.get_metadata(path, 'xattr')
